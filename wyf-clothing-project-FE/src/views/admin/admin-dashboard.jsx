...
export default function AdminDashboard() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            for (const record of records) {
                ...
                formData.append(
                    "keptImages",
                    JSON.stringify(keptByRecord[record.dashboard_id] || [])
                );
                ...
                if (!res.data.success) {
                    errors.push(
                        res.data.message || `Update failed for record ${record.dashboard_id}`
                    );
                } else {
                    updates.push({
                        dashboard_id: record.dashboard_id,
            ...
            setKeptImages((prev) => {
                const deletedIds = updates
                    .filter((u) => u.deleted)
                    .map((u) => u.dashboard_id);
                const remaining = prev.filter(
                    (img) => !deletedIds.includes(img.dashboard_id)
                );
    ...
    return (
        ...
        @media (max-width: 480px) {
          ...
          .footer-msg { justify-content: center; font-size: 12px; }
        }
        ...
        @media (hover: none) {
          .image-card .remove-btn { opacity: 1 !important; }
        }
                        ...
                        <div style={styles.body}>
                            ...
                            <div style={styles.section}>
                                ...
                                {newFiles.length > 0 && (
                                    <div
                                        className="img-grid"
                                        style={{ ...styles.grid, marginTop: 12, marginBottom: 12 }}
                                    >
                                        {newFiles.map((file, i) => (
                                            <div
                                                key={i}
                                ...
                                >
                                    ...
                                    <p style={styles.dropzoneText}>
                                        Drag & drop, or{" "}
                                        <span style={styles.dropzoneStrong}>browse files</span>
                                    </p>
